...
## results 
seasonres <- matrix(nrow=length(which(ress$padj< 0.05)), ncol=6)
seasonres[,1] <- rownames(euk_sort)[which(ress$padj< 0.05)]
for (i in 1:nrow(seasonres)){seasonres[i,2] <- as.character(euk_surf$taxo2[which(euk_surf$OTU==seasonres[i,1])])}
for (i in 1:nrow(seasonres)){seasonres[i,3] <- as.character(euk_surf$taxo3[which(euk_surf$OTU==seasonres[i,1])])}
for (i in 1:nrow(seasonres)){seasonres[i,4] <- as.character(euk_surf$taxo6[which(euk_surf$OTU==seasonres[i,1])])}
for (i in 1:nrow(seasonres)){seasonres[i,5] <- as.character(euk_surf$taxo7[which(euk_surf$OTU==seasonres[i,1])])}
seasonres[,6] <- ress$log2FoldChange[which(ress$padj< 0.05)]

seasonres <- as.data.frame(seasonres)
colnames(seasonres) <- c("OTU","Phylum", "Class", "Genus","Species", "log2Foldchange")

seasonagg <- aggregate(as.numeric(as.character(seasonres$log2Foldchange)), by=list(seasonres$Class, seasonres$Genus, seasonres$Species), 
                       FUN="mean")
...
## results 
condres <- matrix(nrow=length(which(ress$padj< 0.05)), ncol=6)
condres[,1] <- rownames(euk_sort)[which(ress$padj< 0.05)]
for (i in 1:nrow(condres)){condres[i,2] <- as.character(euk_surf$taxo2[which(euk_surf$OTU==condres[i,1])])}
for (i in 1:nrow(condres)){condres[i,3] <- as.character(euk_surf$taxo3[which(euk_surf$OTU==condres[i,1])])}
for (i in 1:nrow(condres)){condres[i,4] <- as.character(euk_surf$taxo6[which(euk_surf$OTU==condres[i,1])])}
for (i in 1:nrow(condres)){condres[i,5] <- as.character(euk_surf$taxo7[which(euk_surf$OTU==condres[i,1])])}
condres[,6] <- ress$log2FoldChange[which(ress$padj< 0.05)]

condres <- as.data.frame(condres)
colnames(condres) <- c("OTU","Phylum", "Class", "Genus","Species", "log2Foldchange")

condagg <- aggregate(as.numeric(as.character(condres$log2Foldchange)), by=list(condres$Class,condres$Genus, condres$Species), 
                     FUN="mean")